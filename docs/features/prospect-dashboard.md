...
| `FR-072` | The Signals tab shall list the account's signals for the service, filtered by question and by status (Counting, Marked wrong, Outdated question), each with question, strength label, confidence word, Quick check or Detailed check, quote, translation, source and age. |
