...
# `API-61` (`GET /health`) is the one contract this task builds; every other row is a stub.
STUB_CONTRACTS = [row for row in REST_CONTRACTS if row.id != "API-61"]
...
async def test_a_declared_contract_not_built_answers_501_not_implemented(
    ...
    response = await client.request(row.method, url)
...
async def test_a_stub_answers_501_even_when_its_body_misses_required_fields(
    ...
    url = f"/api/v1{_fill_path(row)}"
    response = await client.request(row.method, url, json={})
...
async def test_a_malformed_json_body_answers_422_with_the_validation_envelope(
    ...
    response = await client.request(
        row.method, url, content=b"not json", headers={"content-type": "application/json"}
    )
